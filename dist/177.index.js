...
module.exports = __webpack_require__(5133);
...
const stringify = __webpack_require__(1514);
...
const NodeFsHandler = __webpack_require__(1335);
const FsEventsHandler = __webpack_require__(1771);
const {
  EV_ALL,
  EV_READY,
  EV_ADD,
...
__webpack_unused_export__ = FSWatcher;
...
let fsevents;
try {
  fsevents = __webpack_require__(6289);
} catch (error) {
...
const sysPath = __webpack_require__(1017);
const { promisify } = __webpack_require__(3837);
const isBinaryPath = __webpack_require__(1436);
const {
  isWindows,
  isLinux,
  EMPTY_FN,
...
const Native = __webpack_require__(2982);
...
const binaryExtensions = __webpack_require__(329);
...
__webpack_require__.d(__webpack_exports__, {
  ...
  "loadDotenv": () => (/* binding */ loadDotenv),
  "setupDotenv": () => (/* binding */ setupDotenv),
  "watchConfig": () => (/* binding */ watchConfig)
});

// EXTERNAL MODULE: external "node:fs"
var external_node_fs_ = __webpack_require__(7561);
// EXTERNAL MODULE: ./node_modules/pathe/dist/shared/pathe.92c04245.mjs
var pathe_92c04245 = __webpack_require__(7325);
...
;// CONCATENATED MODULE: ./node_modules/rc9/node_modules/destr/dist/index.mjs
...
var node_modules_flat = __webpack_require__(9681);
...
function parse(contents, options = {}) {
  ...
  return options.flat ? config : node_modules_flat.unflatten(config, { overwrite: true });
}
...
function defaultGetFormatWithoutErrors(url, context) {
  if (!dist_hasOwnProperty.call(protocolHandlers, url.protocol)) {
    return null
  }
...
const DEFAULT_URL = (0,external_node_url_.pathToFileURL)(process.cwd());
...
function _resolve(id, options = {}) {
  ...
  if ((0,pathe_92c04245.i)(id) && (0,external_node_fs_.existsSync)(id)) {
    const realPath2 = (0,external_node_fs_.realpathSync)(fileURLToPath(id));
    return (0,external_node_url_.pathToFileURL)(realPath2).toString();
  }
  ...
  const realPath = (0,external_node_fs_.realpathSync)(fileURLToPath(resolved));
  return (0,external_node_url_.pathToFileURL)(realPath).toString();
}
...
async function findFile(filename, _options = {}) {
  ...
  const basePath = (0,pathe_92c04245.r)(options.startingFrom);
  ...
  if (!options.reverse) {
    for (let index = segments.length; index > root; index--) {
      const filePath = (0,pathe_92c04245.j)(...segments.slice(0, index), filename);
...
async function readTSConfig(id, options = {}) {
  ...
  const jsonc = await __webpack_require__.e(/* import() */ 245).then(__webpack_require__.t.bind(__webpack_require__, 245, 19));
...
async function resolvePackageJSON(id = process.cwd(), options = {}) {
  const resolvedPath = (0,pathe_92c04245.i)(id) ? id : await dist_resolvePath(id, options);
...
var chokidar = __webpack_require__(2677);
...
class Hasher extends (/* unused pure expression or super */ null && (BufferedBlockAlgorithm)) {
...
const H = (/* unused pure expression or super */ null && ([
  1779033703,
  -1150833019,
  1013904242,
  -1521486534,
...
const W = (/* unused pure expression or super */ null && ([]));
class SHA256 extends (/* unused pure expression or super */ null && (Hasher)) {
  constructor() {
    super();
    this.reset();
  }
...
async function loadDotenv(options) {
  ...
  const dotenvFile = (0,pathe_92c04245.r)(options.cwd, options.fileName);
...
async function loadConfig(options) {
  options.cwd = (0,pathe_92c04245.r)(process.cwd(), options.cwd || ".");
  ...
  const r = {
    config: {},
    cwd: options.cwd,
    configFile: (0,pathe_92c04245.r)(options.cwd, options.configFile),
    layers: []
  };
  if (options.dotenv) {
    await setupDotenv({
  ...
  if (options.rcFile) {
    if (options.globalRc) {
      Object.assign(
        configRC,
        readUser({ name: options.rcFile, dir: options.cwd })
      );
      ...
      if (workspaceDir) {
        Object.assign(
          configRC,
          read({ name: options.rcFile, dir: workspaceDir })
        );
      }
    }
    Object.assign(
      configRC,
      read({ name: options.rcFile, dir: options.cwd })
    );
  }
  const pkgJson = {};
  if (options.packageJson) {
    ...
    Object.assign(pkgJson, (0,dist_defu/* defu */.ob)({}, ...values));
  }
  r.config = (0,dist_defu/* defu */.ob)(
    options.overrides,
    config,
    configRC,
    pkgJson,
  ...
  if (options.extend) {
    ...
    r.config = (0,dist_defu/* defu */.ob)(r.config, ...r.layers.map((e) => e.config));
  }
  const baseLayers = [
    options.overrides && {
  ...
  if (options.defaults) {
    r.config = (0,dist_defu/* defu */.ob)(r.config, options.defaults);
  }
  return r;
}
...
async function resolveConfig(source, options, sourceOptions = {}) {
  ...
  if (GIT_PREFIXES.some((prefix) => source.startsWith(prefix))) {
    const { downloadTemplate } = await __webpack_require__.e(/* import() */ 232).then(__webpack_require__.bind(__webpack_require__, 7232));
    const url = new URL(source);
    const gitRepo = url.protocol + url.pathname.split("/").slice(0, 2).join("/");
    const name = gitRepo.replace(/[#/:@\\]/g, "_");
    const tmpDir = process.env.XDG_CACHE_HOME ? (0,pathe_92c04245.r)(process.env.XDG_CACHE_HOME, "c12", name) : (0,pathe_92c04245.r)((0,external_node_os_.homedir)(), ".cache/c12", name);
    if ((0,external_node_fs_.existsSync)(tmpDir)) {
      await (0,promises_.rm)(tmpDir, { recursive: true });
    }
  ...
  const ext = (0,pathe_92c04245.e)(source);
  const isDir = !ext || ext === (0,pathe_92c04245.g)(source);
  const cwd = (0,pathe_92c04245.r)(options.cwd, isDir ? source : (0,pathe_92c04245.c)(source));
  if (isDir) {
  ...
  try {
    res.configFile = options.jiti.resolve((0,pathe_92c04245.r)(cwd, source), {
      paths: [cwd]
    });
  } catch {
  }
  ...
  if (options.envName) {
    ...
    if (Object.keys(envConfig).length > 0) {
      res.config = (0,dist_defu/* defu */.ob)(envConfig, res.config);
    }
  }
  res.meta = (0,dist_defu/* defu */.ob)(res.sourceOptions.meta, res.config.$meta);
  delete res.config.$meta;
  if (res.sourceOptions.overrides) {
    res.config = (0,dist_defu/* defu */.ob)(res.sourceOptions.overrides, res.config);
  }
  return res;
}
    ...
    ...new Set(
      (config.layers || []).filter((l) => l.cwd).flatMap((l) => [
        ...["ts", "js", "mjs", "cjs", "cts", "mts", "json"].map(
          (ext) => (0,pathe_92c04245.r)(l.cwd, configFileName + "." + ext)
        ),
        l.source && (0,pathe_92c04245.r)(l.cwd, l.source),
        // TODO: Support watching rc from home and workspace
        options.rcFile && (0,pathe_92c04245.r)(
          l.cwd,
          typeof options.rcFile === "string" ? options.rcFile : `.${configName}rc`
        ),
        options.packageJson && (0,pathe_92c04245.r)(l.cwd, "package.json")
      ]).filter(Boolean)
    )
  ];
  const _fswatcher = (0,chokidar/* watch */.Y)(watchingFiles, {
    ignoreInitial: true,
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "b": () => (/* binding */ relative),
/* harmony export */   "c": () => (/* binding */ dirname),
/* harmony export */   "e": () => (/* binding */ extname),
/* harmony export */   "g": () => (/* binding */ basename),
/* harmony export */   "i": () => (/* binding */ isAbsolute),
/* harmony export */   "j": () => (/* binding */ join),
/* harmony export */   "r": () => (/* binding */ resolve)
/* harmony export */ });
/* unused harmony exports a, d, f, h, k, n, p, s, t */
